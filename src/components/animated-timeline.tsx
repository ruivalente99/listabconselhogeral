...
const defaultStyles: TimelineStyles = {
  lineColor: "#d1d5db",
  activeLineColor: "#8b5cf6", // purple-600
  dotColor: "#d1d5db",
  activeDotColor: "#7c3aed", // purple-700
  dotSize: "1.5rem",
  titleColor: "inherit",
...
export function AnimatedTimeline({
  ...
  const handleHover = (index: number | null) => {
    ...
    onEventHover?.(index !== null && events[index] ? events[index] : null);
  };

  return (
    <div className={`relative py-4 ${className} text-purple-600`}>
...
export default function AnimatedTimelinePage({
  ...
  return (
    <div
      className={cn(
        "container mx-auto rounded-lg bg-background px-8 pt-6 text-fuchsia-900",
        containerClassName,
      )}
    >
      <h1 className="text-2xl font-bold text-fuchsia-900">{title ?? defaultTitle}</h1>
      <AnimatedTimeline
